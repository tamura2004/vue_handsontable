...
namespace :tool do
  ...
  desc "モデル一覧作成"
  task :rspec_scaffold => :environment do
    models = Rails.root.join("app","models").children.each do |path|
      next if path.extname != ".rb"
      model = path.basename(".rb").to_s.classify.constantize

      print "rails g rspec:scaffold "
      print model.name
      begin
        model.columns.each do |col|
          next if col.name =~ /_at$/
  ...
  desc "リクエストスペック一括修正"
  task :modify_request_spec => :environment do
    TEMPLATE = <<-EOD
...
RSpec.describe "Attendances", type: :request do
  ...
  describe "DELETE /attendances/1" do
    ...
    Rails.root.join("spec","requests").children.each do |path|
